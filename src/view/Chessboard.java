...
//
    ...
    public void swapChessComponents(ChessComponent chess1, ChessComponent chess2) {
        ...
        chess1.repaint();
        chess2.repaint();
//        Object[] options1={"骑士","皇后","大象","战车"};
//        int op1 = JOptionPane.showOptionDialog(null, "请选择升级成：", "升级",JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE,null, options1, options1[0]);
        regretChessDataHelp=StoreGame();

        if (regretChessDataHelp.get(7).contains("P")){
            //System.out.println(")))))))))))))))))))))))))))))))))))))))))))))");

//                System.out.println(regretChessDataHelp.get(7));
//            System.out.println(regretChessDataHelp.get(7).charAt(1));
            for(int j=0;j<8;j++) {
